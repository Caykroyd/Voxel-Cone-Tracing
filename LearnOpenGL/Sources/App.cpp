...
void App::glfw_init() {
	...
	glfwSetFramebufferSizeCallback(window, &App::framebuffer_size_callback); // for when the user attempts to resize the window: we in this case set it's size accordingly
...
void App::glad_init(glm::vec4 color = glm::vec4(0.2f, 0.3f, 0.3f, 1.0f)) {
...
void App::exit_error(const std::string& message) {
	App::getInstance().terminate_resources();
...
void App::set_light(const Light l) const {
...
void App::set_material(const Material m) const {
...
double App::delta_time = 0.0;
double App::_time = 0.0;
...
void App::set_scene(Scene& s) const {
...
{
	//App::getInstance().current_scene->camera->setAspectRatio(static_cast<float>(width) / static_cast<float>(height));
...
void App::key_callback(GLFWwindow * windowPtr, int key, int scancode, int action, int mods) {
...
std::string App::toString(glm::vec3 v) {
